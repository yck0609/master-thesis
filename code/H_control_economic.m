...
theta = 48.35; %H无穷控制L2增益
theta_filtering = 30; %H无穷滤波L2增益
...
K_u_initial(:,:,1) = [-2  2   2   2];
K_u_initial(:,:,2) = [-2  2   2   2];
...
norm_P_1_episode(1) = log(trace(P(:,:,1)'*P(:,:,1)) + 1);
norm_P_2_episode(1) = log(trace(P(:,:,2)'*P(:,:,2)) + 1);
norm_K_1_episode(1) = log(trace(K(:,:,1)'*K(:,:,1)) + 1);
norm_K_2_episode(1) = log(trace(K(:,:,2)'*K(:,:,2)) + 1);
...
for episode = 1:episodes
    ...
    while n < 250  %进行耦合Lyapunov方程的求解
        for mode = 1:modes   %对三个模态进行迭代求解
            sigmma_V(:,:,mode) = Pr(mode,1)*V(:,:,1) + Pr(mode,2)*V(:,:,2); %计算按概率加权求和的P
    ...
    norm_P_1_episode(episode + 1) = log(trace(P(:,:,1)'*P(:,:,1)) + 1);
    norm_P_2_episode(episode + 1) = log(trace(P(:,:,2)'*P(:,:,2)) + 1);
    norm_K_1_episode(episode + 1) = log(trace(K(:,:,1)'*K(:,:,1)) + 1);
    norm_K_2_episode(episode + 1) = log(trace(K(:,:,2)'*K(:,:,2)) + 1);
end
...
delta1 = trace(delta(:,:,1)'*delta(:,:,1)+delta(:,:,2)'*delta(:,:,2))
...
legend('$log(\left\|P_{1}\right\|_{2})$','$log(\left\|P_{2}\right\|_{2})$','Interpreter','latex'); %legend在坐标区上添加图例
...
legend('$log(\left\|K_{1}\right\|_{2})$','$log(\left\|K_{2}\right\|_{2})$','Interpreter','latex');
...
for episode_TD = 1:episodes_TD
    ...
    for mode = 1:modes
        ...
        for step_TD=1:steps_TD
            ...
            mode_now = randsrc(1,1,[1 2;Pr(mode_old,:)]); %基于转移概率更新模态
...
for i = 1:modes
    delta_TD(:,:,i) = sigmma_P_TD(:,:,i) - Pr(i,1)*P_TD(:,:,1) - Pr(i,2)*P_TD(:,:,2);
end
delta_TD_norm = trace(delta_TD(:,:,1)'*delta_TD(:,:,1)+delta_TD(:,:,2)'*delta_TD(:,:,2))
...
legend('$\Delta^{(l)}_{1}$','$\Delta^{(l)}_{2}$','Interpreter','latex','Position', ...
...
L_y_initial(:,:,1) = [-0.3 0 0;0 -0.4 0;0 0 -0.4;0 -0.2 0.5];
L_y_initial(:,:,2) = [-0.3 0 0;0 -0.4 0;0 0 -0.4;0 -0.2 0.5];
...
for episode_filtering = 1:episodes_filtering
    ...
    while n < 200  %进行耦合Lyapunov方程的求解
        for mode = 1:modes   %对三个模态进行迭代求解
            sigmma_V_filtering(:,:,mode) = Pr(mode,1)*V_filtering(:,:,1) + Pr(mode,2)*V_filtering(:,:,2); %计算按概率加权求和的P
    ...
    for mode = 1:modes %基于求解的P更新K_u以及K_w
        sigmma_P_filtering(:,:,mode) = Pr(mode,1)*P_filtering(:,:,1) + Pr(mode,2)*P_filtering(:,:,2);  %计算按概率加权求和的P
...
delta_filtering = trace(delta_filtering(:,:,1)'*delta_filtering(:,:,1) + delta_filtering(:,:,2)'*delta_filtering(:,:,2))
...
legend('$log(\left\|L_{1}\right\|_{2})$','$log(\left\|L_{2}\right\|_{2})$','Interpreter','latex');
...
for i = 1:modes
    delta_TD_filtering(:,:,i) = sigmma_P_TD_filtering(:,:,i) - Pr(i,1)*P_TD_filtering(:,:,1) - Pr(i,2)*P_TD_filtering(:,:,2);
end
delta_TD_filtering_norm = trace(delta_TD_filtering(:,:,1)'*delta_TD_filtering(:,:,1) + delta_TD_filtering(:,:,2)'*delta_TD_filtering(:,:,2))
...
%% 仿真跟踪系统
mode_now = randsrc(1,1,[1 2;1/2 1/2]);
for step_test = 1:steps_test
    mode_old = mode_now; %储存当前模态
    mode_now = randsrc(1,1,[1 2;Pr(mode_old,:)]); %基于转移概率更新模态

    tilde_w(:,step_test) = normrnd(0,0.1,[F_col + hat_F_col 1]);
    tilde_w_TD(:,step_test) = normrnd(0,0.1,[F_col + hat_F_col 1]);
