...
S_u_initial(:,:,1) = [-10  10   10   10];
S_u_initial(:,:,2) = [-10  10   10   10];

% val(:,:,1) = -2.0993    2.2100    2.1506    2.2363
% val(:,:,2) = -2.2137    2.2888    2.1965    2.3735
...
norm_S_1_episode(1) = log(trace(S(:,:,1)'*S(:,:,1)) + 1);
norm_S_2_episode(1) = log(trace(S(:,:,2)'*S(:,:,2)) + 1);
...
for episode = 1:episodes
    ...
    norm_S_1_episode(episode+1) = log(trace(S(:,:,1)'*S(:,:,1)) + 1);
    norm_S_2_episode(episode+1) = log(trace(S(:,:,2)'*S(:,:,2)) + 1);
end
...
legend('$log(\left\|S_{1}\right\|_{2})$','$log(\left\|S_{2}\right\|_{2})$','Interpreter','latex');
...
delta_sigmma_P_TD = [0 0]; %用于储存sigmma_P距离最优的差值
delta_S_TD = [0 0]; %用于储存K_u距离最优的差值

episodes_TD = 501; %定义迭代的幕数
steps_TD = 500; %定义每一幕的步数
lambda = 0.999; %定义回报权重
